...
export default function BlogClient({
  ...
  const mappedPosts: BlogPost[] = initialPosts.map((p) => ({
    ...
    date:
      p.published_at?.split("T")[0] || p.created_at?.split("T")[0] || "Recent",
  }));
          ...
          <thead>
            <tr className="border-b border-(--border-subtle) bg-(--bg-elevated)">
              {headers.map((header, idx) => (
                <th
                  key={`${header}-${idx}`}
                  className={`p-4 text-sm font-medium text-(--text-secondary) ${idx === headers.length - 1 ? "text-right" : ""}`}
                >
                  {header}
                </th>
              ))}
